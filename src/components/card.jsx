    ...
    return (
      <>
        <div className='card' onClick={this.handleModal} style={{ backgroundColor: `var(--bg-poke-color-light-${pokemon.types[0].type.name})` }}>
          ...
          {pokemon.types[1] ?
            <div className="card__title" style={{ backgroundColor: `var(--bg-poke-color-dark-${pokemon.types[1].type.name})` }}>
              <img className='card__badge-Icon' src={searchIcon(pokemon.types[1].type.name)} alt="" />
              <span className='card__badge-text'>{pokemon.types[1].type.name}</span>
            </div> : <div className='card__title'>
              <img className='card__title-img' src={PokeballImage} alt="" />
              <span className='card__title-text'>{`#${pokemon.order}`}</span>
            </div>
          }
